...
SELECT A.DATABASE_NAME,
       ...
       B.QUERY_PLAN AS BEFORE_PLAN,
       C.QUERY_PLAN AS AFTER_PLAN
FROM   (SELECT DISTINCT STARTING.DATABASE_NAME,
					    STARTING.QUERY_HASH,
                        STARTING.EXECUTION_COUNT,
                        STARTING.AVG_ELAPSED_TIME                                                               AS BEFORE_AVG_TIME,
                        ENDING.AVG_ELAPSED_TIME                                                               AS CURRENT_AVG_TIME,
                        STARTING.AVG_ELAPSED_TIME - ENDING.AVG_ELAPSED_TIME                                         AS 'TIME_DIFF(ms)',
                        Cast(( STARTING.AVG_ELAPSED_TIME - ENDING.AVG_ELAPSED_TIME ) / CASE ENDING.AVG_ELAPSED_TIME
                                                                               WHEN 0 THEN 1
...
SELECT A.DATABASE_NAME,
       ...
       A.SQL_TEXT,
       B.QUERY_PLAN AS BEFORE_PLAN
FROM   (SELECT DISTINCT ENDING.DATABASE_NAME,
					    ENDING.QUERY_HASH,
                        ENDING.AVG_ELAPSED_TIME AS BEFORE_AVG_TIME,
                        ENDING.SQL_TEXT,
                        ENDING.QUERY_PLAN_HASH  AS BEFORE_PLAN_HASH
        FROM   QUERY_STATS_HASH_VW ENDING
        
        WHERE  ENDING.RUN_NAME = 'Feb_26_2020_804AM'
               ...
               AND NOT EXISTS (SELECT QUERY_HASH
                               ...
                               WHERE  ENDING.QUERY_HASH = STARTING.QUERY_HASH 
                                      ...
                                      AND STARTING.RUN_NAME = 'BASE_to_compare_to')
