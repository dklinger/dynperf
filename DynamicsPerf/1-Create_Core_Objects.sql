...
AS
  ...
  SELECT TOP 100 PERCENT *, (SELECT TOP 1 F.[PHYSICAL_NAME] FROM SQL_DATABASEFILES F WHERE F.[DATABASE_NAME] = S.DATABASE_NAME
  AND F.[FILE_ID] = S.FILE_ID ORDER BY STATS_TIME DESC) AS [PHYSICAL_NAME]
  FROM   PERF_IO_STATS S ORDER BY STATS_TIME DESC
